...
flags.DEFINE_string('env_name', 'maze:10-blocks:10', 'Environment name.')
flags.DEFINE_integer('seed', 0, 'Initial random seed.')
flags.DEFINE_integer('num_trajectory', 200,
...
flags.DEFINE_string('save_dir', ".", 'Directory to save dataset to.')
flags.DEFINE_string('load_dir', ".", 'Directory to load policies from.')
...
def get_env(load_dir, env_name, tabular_obs, max_trajectory_length,
            alpha, seed):
    tf_env, tf_policy = env_policies.get_env_and_policy(
        load_dir, env_name, alpha, env_seed=seed, tabular_obs=tabular_obs)
...
def main(argv):
  ...
  writer= dict()
  traj = data[0]
  writer['observations'] = traj.observation
  writer["actions"] = traj.action
  writer['rewards'] = traj.reward

  pickle_filename = os.path.join(str(save_dir) + "/maze_pickle" + str(env_name))
